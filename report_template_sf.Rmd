...
total_effect_sentence <- if (tolower(params$yreg_type) == "linear") {
  "The total effect was defined as the sum of the direct and indirect effects."
} else if (tolower(params$yreg_type) == "logistic") {
  "The total effect was defined as the product of the direct and indirect effects."
} else {
  "The total effect was defined based on the chosen outcome model."
}

basec_text <- if (!is.null(params$basec_vars) && length(params$basec_vars) > 0) {
  paste0("Models were adjusted for baseline covariates ", paste(params$basec_vars, collapse = ", "), ".")
} else { "" }
